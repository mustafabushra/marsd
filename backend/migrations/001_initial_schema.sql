...
CREATE INDEX idx_tenants_cr_number ON tenants(cr_number);
...
CREATE INDEX idx_reports_reporter_tenant_id ON reports(reporter_tenant_id);
CREATE INDEX idx_reports_target_company_id ON reports(target_company_id);
...
CREATE INDEX idx_review_actions_report_id ON review_actions(report_id);
...
CREATE UNIQUE INDEX idx_watchlist_items_unique ON watchlist_items(tenant_id, company_id);
...
CREATE INDEX idx_business_requests_from_tenant_id ON business_requests(from_tenant_id);
CREATE INDEX idx_business_requests_to_tenant_id ON business_requests(to_tenant_id);
...
CREATE INDEX idx_view_quota_usage_tenant_id ON view_quota_usage(tenant_id);
CREATE UNIQUE INDEX idx_view_quota_usage_unique ON view_quota_usage(tenant_id, period);
...
CREATE TRIGGER business_requests_updated_at_trigger
BEFORE UPDATE ON business_requests FOR EACH ROW
EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER view_quota_usage_updated_at_trigger
...
COMMENT ON TABLE users IS 'Platform and tenant users with role-based access';
