...
/**
 * Bootstrap Script — Initializes Autonomous Agents to build Marsad
 * Runs the core tasks needed to complete the platform
 */
...
╚════════════════════════════════════════════════════════════════════════════╝
  ...
  // ============================================================================
  // PHASE 1: Security & Architecture (Parallel)
  // ============================================================================
  ...
  const securityAuditTask = new TaskBuilder(
    ...
    'Verify complete security posture before development'
  )
  ...
  const dbSetupTask = new TaskBuilder(
    'Database Setup - PostgreSQL + RLS',
    'Configure database with Row-Level Security & encryption'
  )
    .setPriority('critical')
  ...
  const authImplementationTask = new TaskBuilder(
    'Backend Auth System - JWT + Refresh Rotation',
    'Implement bcryptjs passwords + JWT tokens + refresh token rotation'
  )
    .setPriority('critical')
  ...
  // ============================================================================
  // PHASE 2: Backend Core APIs (Dependent on Phase 1)
  // ============================================================================

  console.log('\n\n📋 PHASE 2: Backend API Development (Secured)\n')

  const companySearchAPI = new TaskBuilder(
    'API: POST /companies/search',
    'Company search with full-text search + pagination'
  )
    .setPriority('high')
  ...
  const trustScoreAPI = new TaskBuilder(
    'API: GET /trust-score/:companyId',
    'Trust Score calculation with gating logic'
  )
    .setPriority('high')
    .addDependency(authImplementationTask.id)
    .addSecurityCheck('owasp-top10', 'check', 'high')
  ...
  const reportsAPI = new TaskBuilder(
    'API: POST /reports + GET /reports/:id',
    'Report submission & retrieval with approval workflow'
  )
    .setPriority('high')
    .addDependency(authImplementationTask.id)
    .addSecurityCheck('owasp-top10', 'check', 'high')
  ...
  const watchlistAPI = new TaskBuilder(
  ...
  const searchPageComponent = new TaskBuilder(
    'Frontend: Search Page Component',
    'Company search UI with filters + trust score display'
  )
    .setPriority('high')
    .addSecurityCheck('owasp-top10', 'check', 'high')
    .build()

  const reportPageComponent = new TaskBuilder(
    'Frontend: Trust Report Page',
    'Company report display with 4 states (موثوق/متوسطة/غير كافي/مقفل)'
  )
    .setPriority('high')
    .addSecurityCheck('owasp-top10', 'check', 'high')
    .build()

  const dashboardComponent = new TaskBuilder(
    'Frontend: Company Dashboard',
    'KPI cards + watchlist + recent activity'
  )
    .setPriority('high')
    .addSecurityCheck('owasp-top10', 'check', 'high')
    .build()

  const adminPanelComponent = new TaskBuilder(
    'Frontend: Admin Dashboard',
    'Report review queue + company management + user control'
  )
    .setPriority('medium')
    .addSecurityCheck('owasp-top10', 'check', 'high')
    .build()
  ...
  const integrationTests = new TaskBuilder(
    'Integration Tests: Full E2E Flows',
    'Login → Search → View Report → Gating → Watchlist'
  )
    .setPriority('critical')
    .addSecurityCheck('owasp-top10', 'check', 'high')
    .build()

  const securityTests = new TaskBuilder(
    'Security Tests: OWASP Penetration',
    'SQL Injection + XSS + CSRF + IDOR + Multi-tenant breach attempts'
  )
    .setPriority('critical')
  ...
  const performanceTests = new TaskBuilder(
    'Performance Tests: Load & Benchmarks',
    'Frontend build size + Search response time + Score calculation'
  )
    .setPriority('high')
    .addSecurityCheck('owasp-top10', 'check', 'high')
    .build()
