...
`);
    const infraTask = new TaskBuilder('INFRA-01: AWS Infrastructure + Cloudflare + CI/CD', 'Deploy VPC, ECS, RDS, ElastiCache, S3, CloudWatch, GitHub Actions')
        .setPriority('critical')
        .setPhase(1)
        .addSecurityCheck('owasp-top10', 'Infrastructure security hardening', 'high')
        .build();
    const dbTask = new TaskBuilder('DB-01: Prisma Schema + PostgreSQL + Row-Level Security', `Design and implement all ${Object.keys(MARSAD_SPECIFICATION).includes('ERD') ? '17' : 'N'} tables from ERD with RLS policies`)
        .setPriority('critical')
        .setPhase(1)
        .addSecurityCheck('multi-tenant', 'RLS isolation on all tenant_id columns', 'critical')
        .build();
    const authTask = new TaskBuilder('AUTH-01: JWT Authentication + Refresh Token Rotation', 'Implement: Argon2id hashing, 15min access tokens, 7day refresh rotation, account lockout')
        .setPriority('critical')
        .setPhase(1)
        ...
        .build();
    const marketingTask = new TaskBuilder('FRONTEND-01: Marketing Website (7 screens)', `Build: Landing (الرئيسية), About (عن), Pricing (الباقات), FAQ, Contact, Register, Login`)
        .setPriority('high')
        .setPhase(1)
...
`);
    const searchTask = new TaskBuilder('API-01: Company Search (FR-05)', 'Implement GET /companies with full-text search by name/CR/sector/city with pagination')
        .setPriority('critical')
        .setPhase(2)
        ...
        .build();
    const reportPageTask = new TaskBuilder('FRONTEND-02: Trust Report Page (FR-06 - 4 states)', 'Display: موثوق (5+ reports) | أولي (2-4 reports) | غير كافي (0-1) | مقفول (free tier)')
        .setPriority('critical')
        .setPhase(2)
        .addDependency(searchTask.id)
        .addSecurityCheck('owasp-top10', 'XSS prevention in report display', 'high')
        .build();
    const reportWizardTask = new TaskBuilder('FRONTEND-03: Report Submission Wizard (FR-08 - 4 steps)', 'Step 1: Deal info | Step 2: Payment commitment | Step 3: Delay info | Step 4: Documents + Attestation')
        .setPriority('critical')
        .setPhase(2)
        ...
        .build();
    const trustScoreTask = new TaskBuilder('ENGINE-01: Trust Score Calculation Engine (FR-10)', `Formula: TrustScore = 0.30×S_official + 0.70×S_community (0-100 range)`)
        .setPriority('critical')
        .setPhase(2)
...
`);
    const reviewQueueTask = new TaskBuilder('ADMIN-01: Report Review Queue (FR-09)', 'Admin interface to approve/reject/request-info on pending reports with decision logging')
        .setPriority('critical')
        .setPhase(3)
        ...
        .build();
    const billingTask = new TaskBuilder('BILLING-01: Subscriptions + Payment Gateway (FR-16)', 'Support 4 plans (Free/Basic/Pro/Enterprise) with Moyasar/Tap integration')
        .setPriority('critical')
        .setPhase(3)
        .addSecurityCheck('owasp-top10', 'PCI compliance for payment handling', 'high')
        .build();
    const watchlistCompareTask = new TaskBuilder('FEATURE-01: Watchlists (FR-12) + Compare (FR-13)', 'Watchlist with notifications on score changes + Compare up to 3 companies side-by-side')
        .setPriority('high')
        .setPhase(3)
        .addSecurityCheck('multi-tenant', 'Watchlist isolation per tenant', 'high')
        .build();
    const businessRequestsTask = new TaskBuilder('FEATURE-02: Business Requests (FR-14)', 'Send/receive business opportunities between tenant companies')
        .setPriority('medium')
        .setPhase(3)
...
`);
    const e2eTask = new TaskBuilder('QA-01: E2E Tests (Golden Journeys)', 'Test: signup→subscribe→search→report(4 states)→upload→approve→score updates')
        .setPriority('critical')
        .setPhase(4)
        .addSecurityCheck('owasp-top10', 'E2E test coverage for security vectors', 'high')
        .build();
    const securityAuditTask = new TaskBuilder('SECURITY-01: Full Security Audit (OWASP + RLS + Anonymity)', 'Verify: All 10 OWASP vectors, RLS isolation, Reporter anonymity (0% exposure), Audit logging')
        .setPriority('critical')
        .setPhase(4)
        ...
        .build();
    const perfUATTask = new TaskBuilder('QA-02: Performance Tuning + Client UAT', 'Achieve: P95 ≤300ms, Page load ≤2.5s, Client sign-off')
        .setPriority('critical')
        .setPhase(4)
        .addSecurityCheck('owasp-top10', 'Performance testing security baseline', 'high')
        .build();
    const launchTask = new TaskBuilder('LAUNCH-01: Production Deployment (GO LIVE)', 'Deploy to production, monitor, hand-off to client on 2026-08-03')
        .setPriority('critical')
        .setPhase(4)
