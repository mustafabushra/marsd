...
export class SecurityEngineer extends BaseAgent {
    ...
    async performSecurityAudit(task) {
        ...
        task.securityChecklist.push(this.requireSecurityCheck('owasp-top10', 'OWASP Top 10 Coverage', 'critical'), this.requireSecurityCheck('multi-tenant', 'Tenant Isolation Verified', 'critical'), this.requireSecurityCheck('auth', 'Auth Mechanisms Verified', 'critical'), this.requireSecurityCheck('crypto', 'Encryption Standards Met', 'critical'), this.requireSecurityCheck('logging', 'Sensitive Data Never Logged', 'critical'));
        const checks = [
    ...
    async preventInjection(task) {
        this.logInfo(`🛡️ Injection Prevention: ${task.description}`);
        task.securityChecklist.push(this.requireSecurityCheck('injection', 'SQL Injection Prevention', 'critical'), this.requireSecurityCheck('injection', 'XSS Prevention', 'critical'), this.requireSecurityCheck('injection', 'Command Injection Prevention', 'critical'), this.requireSecurityCheck('injection', 'LDAP Injection Prevention', 'critical'), this.requireSecurityCheck('injection', 'Template Injection Prevention', 'critical'));
    ...
    async enforceMultiTenantIsolation(task) {
        this.logInfo(`🔐 Multi-Tenant Isolation: ${task.description}`);
        task.securityChecklist.push(this.requireSecurityCheck('multi-tenant', 'Tenant ID Validation', 'critical'), this.requireSecurityCheck('multi-tenant', 'RLS Policies Active', 'critical'), this.requireSecurityCheck('multi-tenant', 'No Cross-Tenant Data Leakage', 'critical'));
        const mtChecks = [
    ...
    async implementEncryption(task) {
        this.logInfo(`🔐 Encryption Implementation: ${task.description}`);
        task.securityChecklist.push(this.requireSecurityCheck('crypto', 'HTTPS/TLS 1.3', 'critical'), this.requireSecurityCheck('crypto', 'Password Hashing (bcryptjs)', 'critical'), this.requireSecurityCheck('crypto', 'Data at Rest Encryption', 'critical'), this.requireSecurityCheck('crypto', 'No Custom Crypto', 'critical'));
        const cryptoChecks = [
    ...
    async enforceRowLevelSecurity(task) {
        this.logInfo(`🔐 Row-Level Security: ${task.description}`);
        task.securityChecklist.push(this.requireSecurityCheck('multi-tenant', 'RLS Policies Defined', 'critical'), this.requireSecurityCheck('multi-tenant', 'RLS Tested', 'critical'));
        const rlsChecks = [
    ...
    async genericSecurityTask(task) {
...
//# sourceMappingURL=SecurityEngineer.js.map